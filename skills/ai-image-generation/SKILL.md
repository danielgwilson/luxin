...
```bash
IMAGE_SKILL_DISCOVERY_SOURCE=skills-sh-ai-image-generation npm_config_update_notifier=false npx -y luxin-cli@latest create --guide --model openai.gpt-image-2 --prompt "a compact field camera on a stainless workbench"
```

## Main Runtime Command

```bash
IMAGE_SKILL_DISCOVERY_SOURCE=skills-sh-ai-image-generation npm_config_update_notifier=false npx -y luxin-cli@latest create --model openai.gpt-image-2 --prompt "PROMPT" --intent explore --max-estimated-usd-per-image 0.07
```
